...
class Bayes_Classifier:
    ...
    def __init__(self, trainDirectory="db_txt_files/"):
        ...
        self.pos_files = 0
        # load the data
        if os.path.isfile('pos_dic.pkl') and os.path.isfile('neg_dic.pkl'):
            self.pos_dic = self.load('pos_dic.pkl')  
            self.neg_dic = self.load('neg_dic.pkl')
            self.neg_files = self.load('neg_files.pkl')
            self.pos_files = self.load('pos_files.pkl')
        else:
            self.train()
    ...
    def train(self):
        ...
        for file in IFileList:
            ...
            if stars == '1':
                ...
                for word in text:
                    self.neg_dic[str(word.lower())] += 1

            elif stars == '5':
                self.pos_files += 1
                text = self.tokenize(self.loadFile(os.path.join(self.trainDirectory, file)))
                for word in text:
                    self.pos_dic[str(word.lower())] += 1

        self.save(self.neg_dic, 'neg_dic.pkl')
        self.save(self.pos_dic, 'pos_dic.pkl')
        self.save(self.neg_files, 'neg_files.pkl')
        self.save(self.pos_files, 'pos_files.pkl')
    ...
    def classify(self, sText):
        ...
        for w in self.tokenize(sText):
            ...
            pos += math.log10((self.pos_dic[w] + 1.0) / self.total_positive)
            neg += math.log10((self.neg_dic[w] + 1.0) / self.total_negative)
